...
Payments are made on the configured network (`--network`) in the configured asset (SBC, or `--sbc`); offers on other networks or in other assets are refused before anything is signed. Both x402 v1 and v2 are supported, selected automatically from the server's advertised `x402Version`:
