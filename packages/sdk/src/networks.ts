 ...
 *
 * Chain identity (id, RPC, explorer, native currency) is viem's own `radius` /
 * `radiusTestnet` chain definitions (`viem/chains`), re-exported here as
 * `radiusMainnetChain` / `radiusTestnetChain` so the same objects feed viem clients directly. A `RadiusNetwork` wraps one of those chains
 * with the Radius-specific pieces x402 needs (facilitator, faucet, payment asset)
 * and exposes a few fields derived from the chain for convenience.
 *
...
import { radius, radiusTestnet as radiusTestnetViem } from 'viem/chains';
...
/** Radius mainnet (id 723487): viem's built-in definition, re-exported. */
export const radiusMainnetChain: Chain = radius;

/** Radius testnet (id 72344): viem's built-in definition, re-exported. */
export const radiusTestnetChain: Chain = radiusTestnetViem;
