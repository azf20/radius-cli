 ...
 *
 * Chain identity (id, RPC, explorer, native currency) lives in viem `Chain`
 * objects — `radiusMainnetChain` / `radiusTestnetChain` — so the same definitions
 * can feed viem clients directly. A `RadiusNetwork` wraps one of those chains
...
/** Native gas token on every Radius instance. */
const RUSD = { name: 'Radius USD', symbol: 'RUSD', decimals: 18 } as const;

/** Radius mainnet as a viem chain (id 723487). */
export const radiusMainnetChain: Chain = defineChain({
  id: 723487,
  name: 'Radius',
  nativeCurrency: RUSD,
  rpcUrls: { default: { http: ['https://rpc.radiustech.xyz'] } },
  blockExplorers: { default: { name: 'Radius Dashboard', url: 'https://network.radiustech.xyz' } },
});

/** Radius testnet as a viem chain (id 72344). */
export const radiusTestnetChain: Chain = defineChain({
  id: 72344,
  name: 'Radius Testnet',
  nativeCurrency: RUSD,
  rpcUrls: { default: { http: ['https://rpc.testnet.radiustech.xyz'] } },
  blockExplorers: { default: { name: 'Radius Dashboard', url: 'https://testnet.radiustech.xyz' } },
  testnet: true,
});
