...
describe('offer selection', () => {
  ...
  it('picks the cheapest compatible offer and skips incompatible ones', async () => {
    stubRpc();
    ...
    expect(offers[0].amount).toBe('13000');
    expect(fromB64(server.requests[1].headers.get('payment-signature')!).accepted).toEqual(EXACT_EIP3009);
  });

  it('surfaces a 402 after payment as payment_rejected', async () => {
    const server = seller(v2(EXACT_EIP3009), () => new Response(null, { status: 402, headers: { 'PAYMENT-REQUIRED': b64({ ...v2(EXACT_EIP3009), error: 'insufficient_funds' }) } }));
    await rejects(buyer(server)(RESOURCE_URL), 'payment_rejected', /insufficient_funds/);
  });
});
